...
import { attachInsightsTakeover } from '../lw-insights-dashboard/insightsTakeover.js';
...
import { mapHit, openPostDetail } from '../lw-blog-list-static/lw-blog-list-static.js';
import '../lw-blog-overview/lw-blog-overview.js';
import '../lw-searches-panel/lw-searches-panel.js';
import { DEFAULT_EXAMPLES, normalizeExample } from '../lw-searches-panel/lw-searches-panel.js';
...
//   unblocked           (String)
//
// ATTRIBUTES — insights dashboard (the full-screen "View Insights" takeover):
//   index-logical-name  (String)  search-analytics index to query; empty shows
//                                 the dashboard's own bundled sample data
//
// ATTRIBUTES — header:
//   logo-src            (String)
//   bar-logo-src        (String)
...
export class LwSummaryPage extends LitElement {
  ...
  static properties = {
    ...
    // insights dashboard (full-screen "View Insights" takeover)
    indexLogicalName: { attribute: 'index-logical-name' },
    ...
    _llm: { state: true },
    // Settings → Overview → "AI Answer". When false, search skips the LLM
    // overview entirely and hits the plain (non-streaming) search endpoint.
    _aiAnswer: { state: true },
    // Settings → Dev Settings → "Show Summary". Forwarded to <lw-blog-list-static>
    // as `show-summary`, which unclamps each row's summary text.
    ...
    // Compare column mode (Settings → Compare, sub-toggle of Side-by-Side
    // View): 'keyword' (live keyword search, ratio 0) or 'ss' (a stored
    // site-search screenshot). Only meaningful while _sideBySide is true.
    _compareMode: { state: true },
    // Screenshot for the example currently selected in "SS vs AI Search"
    // mode — set when the query was picked from the Searches panel and that
    // example carries an `image`; null otherwise (typed queries, history
    // picks, or an example with no screenshot yet).
    _selectedImage: { state: true },
    // Full-size screenshot overlay ('ss' compare mode).
    _ssZoomOpen: { state: true },
  ...
  static styles = css`
    ...
    /* 'SS vs AI Search' mode's left column — a stored site-search screenshot. */
    .compare-ss-image {
      display: block;
      width: 100%;
      height: auto;
    ...
    /* Screenshot + its "view full size" button. */
    .compare-ss-figure { position: relative; }
    ...
    .compare-ss-zoom:hover { background: #ffffff; color: #ee6f2b; }

    @keyframes ss-modal-fade {
      from { opacity: 0; }
      to   { opacity: 1; }
    }
    ...
    /* Full-size screenshot overlay, opened by the button above. */
    .ss-modal {
      animation: ss-modal-fade 0.18s ease-out both;
      position: fixed;
      inset: 0;
      z-index: 1000;
      display: flex;
    ...
    .ss-modal-img {
      animation: ss-modal-zoom 0.24s cubic-bezier(0.2, 0.8, 0.3, 1) both;
      display: block;
      width: 100%;
      max-width: 1400px;
      max-height: 90vh;
    ...
    .ss-modal-close:hover { background: #ffffff; }

    @media (max-width: 600px) {
      .ss-modal { padding: 1rem; }
    }

    @media (prefers-reduced-motion: reduce) {
      .ss-modal,
      .ss-modal-img { animation: none; }
    }
  ...
  constructor() {
    ...
    this._compareMode = 'ss';
    this._selectedImage = null;
    this._ssZoomOpen = false;
    this._searchHistory = [];
  ...
  connectedCallback() {
    ...
    this._onSemanticRatioChange = (e) => {
      ...
      // Stored only — the new ratio applies to the next search the user runs
      // rather than re-running the current one.
    };
    window.addEventListener('lw-settings-semantic-ratio-change', this._onSemanticRatioChange);

    this._onZoomKeydown = (e) => {
      if (e.key === 'Escape' && this._ssZoomOpen) this._ssZoomOpen = false;
    };
    window.addEventListener('keydown', this._onZoomKeydown);
    ...
    // Max Items Returned (comprehensive settings event).
    this._onSettingsChange = (e) => {
      const d = e.detail || {};
      const items = Number(d.totalItems);
      if (items) this._totalItems = items;
      // AI Answer on/off switches which endpoint the search uses (see
      // _runSummary); it takes effect on the next search.
      this._aiAnswer = !!d.aiAnswer;
      // "Show Summary" only changes how the list renders — no re-fetch;
      // assigning the reactive prop re-renders.
      this._showSummary = !!d.showSummary;
      // Compare toggle + mode ('keyword' live search vs 'ss' stored screenshot).
      const wasCompare = this._sideBySide;
      const wasMode = this._compareMode;
      this._sideBySide = !!d.sideBySide;
      this._compareMode = d.compareMode || 'ss';
      // Entering keyword mode (compare just turned on, or switched over from
      // ss mode) with no keyword data yet — fetch it now. Not a re-run of the
      // active search: this column is newly visible and has no data at all, so
      // without it the pane renders empty.
      const enteringKeywordMode =
        this._sideBySide && this._compareMode === 'keyword' && (!wasCompare || wasMode !== 'keyword');
      if (enteringKeywordMode && this._searchQuery && !this._keywordHits) {
        this._fetchKeywordResults(this._searchQuery);
      }
      // Deliberately no re-run of the active search — every setting above
      // applies to the next search the user runs.
    };
    window.addEventListener('lw-settings-change', this._onSettingsChange);

    // <lw-insights-bar>'s "View Insights" button dispatches
    // 'lw-insights-changed' instead of opening its own drawer — see its
    // class docs. attachInsightsTakeover mounts the full-screen dashboard
    // outside this component's shadow root; see its own docs for why, and
    // for why it's shared rather than reimplemented here (<lw-blog> needs
    // the exact same wiring).
    this._detachInsightsTakeover = attachInsightsTakeover(() => ({
      indexLogicalName: this.indexLogicalName,
  ...
  // UNUSED right now but DO NOT REMOVE — see _fetchSummaryStream.
  ...
  _onSearchChange(e) {
    ...
    // Typing invalidates any screenshot from a previously picked example —
    // only an explicit Searches-panel pick (_pickSuggestion) sets one.
    this._selectedImage = null;
  ...
  // `image` is only ever passed by the Searches panel's Examples tab (see the
  // search-pick handler below) — every other caller (chips, History picks)
  // omits it, correctly clearing any screenshot left over from a prior pick.
  _pickSuggestion(text, image = null) {
  ...
  get _exampleList() {
    const list = (Array.isArray(this.examples) && this.examples.length) ? this.examples : DEFAULT_EXAMPLES;
    return list.map(normalizeExample);
  }
  ...
  // The overview's thumbs. Held on the page for now — no feedback endpoint
  // exists yet, so wire the API call here when there is one.
  _onOverviewFeedback(e) {
    this._overviewFeedback = e.detail.value;   // 'up' | 'down' | null
  }

  // The overview's citation link (Citations → Links) when the cited article has
  // no external url. Opens the top-ranked source — the first of the Further
  // Reading blogs behind the answer — in the detail view, taking the same route
  // a click on that row takes.
  _onCitationClick() {
    const hit = this._summaryHits?.[0];
    if (!hit) return;
    openPostDetail(mapHit(hit, this._showSummary), this.detailUrl, { newTab: true });
  }
  ...
  _runSummary({ refetchKeyword = true } = {}) {
    ...
    if (this._aiAnswer) {
      // Part-level attribution: calls the non-streaming /summary endpoint,
      // which returns per-section citations (see _fetchSummary /
      // _mapAnswerToParagraphs). /summary/stream is intentionally NOT called
      // here anymore — it's kept below, unused, for future use.
      this._fetchSummary(this._searchQuery);
    } else {
      // AI Answer is off — skip the LLM overview entirely and hit the plain
      // (non-streaming) search endpoint, same one the landing list uses.
      this._fetchPlainResults(this._searchQuery);
    }
    // Compare mode also runs a keyword search (semanticRatio 0) for the left
    // column — but only in 'keyword' mode; 'ss' mode shows a stored
    // screenshot instead (see _selectedImage) and needs no fetch. Also
    // skipped when only the ratio changed (refetchKeyword=false — keyword is
    // ratio-independent).
    if (this._sideBySide && this._compareMode === 'keyword' && refetchKeyword) {
      this._fetchKeywordResults(this._searchQuery);
    }
  }

  // POST /search/{index}/summary — non-streaming. Returns part-level
  // citations: answer[] is a list of sections, each with its own text and its
  // own citations array (indices into the top-level hits array). This is the
  // active call for the AI column (see _runSummary above). Modeled on
  // _fetchPlainResults below for its abort-controller / loading-state /
  // finally conventions.
  async _fetchSummary(query) {
    const url = this.summaryUrl;
    if (!url || !query) return;

    // Shares the summary controller — only one of _fetchSummary /
    // _fetchPlainResults is ever in flight for the AI column at a time.
    if (this._summaryAbortCtrl) this._summaryAbortCtrl.abort();
    const ctrl = new AbortController();
    ...
    try {
      const res = await fetch(url, {
        ...
        headers: { 'Content-Type': 'application/json', 'X-API-KEY': this.apiKey },
        body: JSON.stringify({
          query, filter: {}, limit: this._totalItems || DEFAULT_PAGE_LIMIT,
          semanticRatio: this._semanticRatio,
          modelProvider: this._modelProvider, llmUsed: this._llm,
        }),
        signal: ctrl.signal,
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      ...
      const hits = Array.isArray(data.hits) ? data.hits : [];
      this._summaryHits = hits;
      // answer can be null (LLM skipped due to quota) — hits still render via
      // the list, just with no overview section.
      this._summaryParagraphs = this._mapAnswerToParagraphs(data.answer, hits);
      this._isSearchResult = true;
      this._saveState();
    } catch (err) {
      if (err.name === 'AbortError') return;
      // Transient failure — keep the last good results on screen instead of
  ...
  // Maps the /summary response's per-section `answer` + top-level `hits` into
  // <lw-blog-overview>'s paragraphs shape: [{ text, citation?: { label,
  // articles } }]. `answer: null` (LLM skipped) yields no overview
  // paragraphs. A section with no citations (e.g. the canned "no relevant
  // information" message) yields a paragraph with no `citation` key, which
  // <lw-blog-overview> already renders with no marker.
    ...
    return answer.map(section => {
      ...
      const articles = citations
        ...
        .map(h => ({
          ...
          // opening the wrong page. `||` not `??` — hits can also carry an
          // empty url string.
  ...
  // UNUSED right now but DO NOT REMOVE — part of the /summary/stream path,
  // kept intentionally for future use. /summary (non-streaming, see
  // _fetchSummary above) replaced this as the active call in _runSummary()
  // because it now returns part-level citations that /summary/stream does
  // not. If streaming is reintroduced later, this is the starting point.
  //
  ...
  async _fetchSummaryStream(query) {
    ...
    try {
      const res = await fetch(url, {
        ...
        // limit = "Max Items Returned" setting, applied to the stream's own
  ...
  // POST /search/{index} — plain (non-streaming) search, used instead of
  // /summary/stream when "AI Answer" is off. Same endpoint + request shape as
  // the landing list (_fetchDefaultList): hits only, no LLM overview.
  async _fetchPlainResults(query) {
    const url = this._searchUrl;
    if (!url || !query) return;

    // Shares the summary controller — only one of _fetchSummaryStream /
    // _fetchPlainResults is ever in flight for the AI column at a time.
    if (this._summaryAbortCtrl) this._summaryAbortCtrl.abort();
    const ctrl = new AbortController();
    this._summaryAbortCtrl = ctrl;
    ...
    try {
      const res = await fetch(url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-API-KEY': this.apiKey },
        body: JSON.stringify({ query, filter: {}, page: 1, limit: this._totalItems || DEFAULT_PAGE_LIMIT, semanticRatio: this._semanticRatio }),
        signal: ctrl.signal,
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      // Apply even when empty — a genuine "no results" answer.
      this._summaryHits = Array.isArray(data.hits) ? data.hits : [];
      // No overview text from this endpoint — clear any left over from a
      // previous AI-Answer-on search.
      this._summaryText = '';
      this._summaryParagraphs = [];
      this._isSearchResult = true;
      this._saveState();
  ...
  // UNUSED right now but DO NOT REMOVE — the /summary/stream path's citation
  // bolt-on (see _fetchSummaryStream). Not called by the active /summary
  // path (_fetchSummary), which already gets real per-section citations from
  // the backend via _mapAnswerToParagraphs.
  ...
  get _paragraphsWithCitation() {
    ...
    const articles = (this._summaryHits ?? []).slice(0, 10).map(h => ({
      ...
      image: (h.imageUrl ?? '').replace(/^\/\//, 'https://'),
      // Same protocol-relative fix the image above gets: a crawled `//host/path`
      // url left raw resolves against the demo host, opening the wrong page.
      // `||` not `??` — hits can also carry an empty url string.
      url: (h.url || '').replace(/^\/\//, 'https://') || '#',
    }));
    if (!articles.length) return paras;

    return paras.map((p, i) => i === paras.length - 1
      ? { ...p, citation: { label: this._citationLabel(articles), articles } }
      : p);
  }
  ...
  disconnectedCallback() {
    ...
    window.removeEventListener('keydown', this._onZoomKeydown);
    window.removeEventListener('lw-settings-change', this._onSettingsChange);
    this._detachInsightsTakeover?.();
    if (this._summaryAbortCtrl) this._summaryAbortCtrl.abort();
    if (this._resultsAbortCtrl) this._resultsAbortCtrl.abort();
    if (this._keywordAbortCtrl) this._keywordAbortCtrl.abort();
  ...
  render() {
    // AI Answer off hides the overview everywhere: this gate covers the simple
    // search view, the compare grid has its own (see the overview row).
    const showSummary = this._aiAnswer && this._searchQuery &&
      ...
      <div class="summary-wrapper">
        ...
        : html`
                <lw-blog-overview
                  ...
                  .paragraphs=${this._summaryParagraphs}
                  show-feedback
                  @overview-feedback=${this._onOverviewFeedback}
    ...
    const renderAiList = (hideHeader, hideCategory = false, hideSidebar = false) => html`
      <lw-blog-list-static
        .hits=${this._summaryHits}
        ...
        ?show-summary=${this._showSummary}
    ...
    const aiList = renderAiList(this._isSearchResult, true, true);
    ...
    // 'ss' mode swaps the keyword column for a stored site-search screenshot
    // of the selected example — no live fetch, no result count.
    const isSsMode = this._compareMode === 'ss';
    ...
    // 'ss' mode's left column. The screenshot sits in the overview row, beside
    // the AI Overview, so the two columns' content starts on the same line
    // instead of the shot being pushed down level with "Further Reading".
    const ssImageTpl = this._selectedImage
      ? html`
          <div class="compare-ss-figure">
            <img class="compare-ss-image" src=${this._selectedImage}
                 alt="Site search screenshot for “${this._searchQuery}”" />
            <button class="compare-ss-zoom" title="View full size"
                    aria-label="View the site-search screenshot full size"
                    @click=${() => { this._ssZoomOpen = true; }}>
        ...
        `
      : html`<div class="compare-note compare-note-list">No site-search screenshot is available for this query yet.</div>`;

    // Shown in the results row instead, since the overview row only renders
    // once a query exists — before that there is no screenshot to sit beside.
    const ssPromptTpl = html`<div class="compare-note compare-note-list">Pick an example from the Searches panel to see its site-search screenshot.</div>`;
    ...
    // 'ss' mode with AI Answer off: the left column holds nothing but the
    // screenshot, so the AI column's results share that row and start level
    // with the top of the shot instead of below it.
    const ssResultsAtTop = isSsMode && !this._aiAnswer && !!this._searchQuery;
    const compareSection = html`
      <div class="compare-tabs" role="tablist">
        <button class="compare-tab ${isAiTab ? 'is-active' : ''}"
          ...
          ${isSsMode ? 'With AI Search' : 'AI Search'} (${aiListCount})
        </button>
        <button class="compare-tab ${!isAiTab ? 'is-active' : ''}"
                role="tab" aria-selected=${!isAiTab}
                @click=${() => this._setCompareTab('keyword')}>
          ${isSsMode ? 'Current Site Search' : `Keyword Search (${keywordCount})`}
        </button>
      </div>

      <div class="compare-grid ${isAiTab ? 'show-ai' : 'show-keyword'}">
        <div class="compare-col-title col-kw">${isSsMode ? 'Current Site Search' : 'Keyword Search'}</div>
        <div class="compare-col-title col-ai">${isSsMode ? 'With AI Search' : 'AI Search'}</div>

        ${ssResultsAtTop ? html`
          <div class="col-kw">${ssImageTpl}</div>
          <div class="col-ai">
            ${showCompareLabels ? html`<div class="compare-label">Results (${hitCount})</div>` : nothing}
            ${this._isSearchResult && hitCount === 0 ? aiEmptyTpl : aiList}
          </div>
        ` : html`
          ${this._searchQuery && (this._aiAnswer || isSsMode) ? html`
            <div class="col-kw">${isSsMode ? ssImageTpl : (this._aiAnswer ? keywordOverviewTpl : nothing)}</div>
            <div class="col-ai">${this._aiAnswer ? aiOverviewTpl : nothing}</div>
          ` : nothing}

          ${showCompareLabels ? html`
            ${isSsMode
            ? html`<div class="col-kw"></div>`
            : html`<div class="compare-label col-kw">Results (${keywordCount})</div>`}
            <div class="compare-label col-ai">Results (${hitCount})</div>
          ` : nothing}

          <div class="col-kw">${isSsMode ? (this._searchQuery ? nothing : ssPromptTpl) : keywordListTpl}</div>
    ...
    const listSection = html`
      ${this._aiAnswer && this._isSearchResult && hitCount
    ...
    // desktop via CSS.
    const example = this._exampleList[this._exampleIndex] ?? { query: '', image: null };
    const exampleCarousel = (this._sideBySide && this._exampleList.length) ? html`
      <div class="example-carousel">
        <button class="example-chip" @click=${() => this._pickSuggestion(example.query, example.image)}>
    ...
    const mainContent = html`
      <lw-blog-search
        ...
        .searchTime=${(this._summaryLoading || !this._aiAnswer) ? null : this._searchTime}
    ...
    // Full-size screenshot. Clicking the backdrop or the close button dismisses
    // it; the image itself stops the click so it stays open.
    const ssZoomModal = (this._ssZoomOpen && this._selectedImage) ? html`
      <div class="ss-modal" role="dialog" aria-modal="true"
           @click=${() => { this._ssZoomOpen = false; }}>
        <button class="ss-modal-close" aria-label="Close"
                @click=${() => { this._ssZoomOpen = false; }}>&times;</button>
        <img class="ss-modal-img" src=${this._selectedImage}
             alt="Site search screenshot for “${this._searchQuery}”"
    ...
    return html`
      ...
      ${this._sideBySide ? html`
        <lw-searches-panel
          ...
          @search-pick=${(e) => this._pickSuggestion(e.detail.value, e.detail.image)}
        ></lw-searches-panel>
      ` : nothing}
